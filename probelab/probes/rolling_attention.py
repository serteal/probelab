...
class RollingAttention(BaseProbe):
    ...
    def initialize(self, sequences: torch.Tensor, mask: torch.Tensor | None = None, labels: torch.Tensor | None = None) -> "RollingAttention":
    ...
    def forward(self, sequences: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if seq_len <= self.window_size:
            ...
            weighted = (attn * vals * m.unsqueeze(-1).to(attn.dtype)).sum(dim=1)
            return self.output(weighted).squeeze(-1)

        w = self.window_size
        logits_win = logits.unfold(1, w, 1)
        vals_win = vals.unfold(1, w, 1)
        mask_win = m.unfold(1, w, 1)
        logits_win = logits_win.masked_fill(~mask_win.unsqueeze(2), float("-inf"))
        attn_win = torch.nan_to_num(torch.softmax(logits_win, dim=-1), nan=0.0)
        window_means = (attn_win * vals_win).sum(dim=-1)
        valid_windows = mask_win.any(dim=-1)
        window_means = window_means.masked_fill(~valid_windows.unsqueeze(-1), float("-inf"))
        head_maxes = window_means.max(dim=1).values
        head_maxes = torch.nan_to_num(head_maxes, nan=0.0, posinf=0.0, neginf=0.0)
        return self.output(head_maxes).squeeze(-1)

    def fit(self, X: Activations, y: list | torch.Tensor, **kwargs) -> "RollingAttention":
