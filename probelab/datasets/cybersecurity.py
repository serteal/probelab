...
@_register_dataset("cyberrisk_qa", Topic.CYBERSECURITY, "Synthetic cyber-risk Q&A from abliterated model")
def cyberrisk_qa(path: str) -> Dataset:
    """Cyber-risk Q&A generated by an abliterated model (gemma-3-27b-it).

    Expert-level offensive cybersecurity Q&A pairs across ~20 topics.

    Args:
        path: Directory containing base_qa_pairs.json (and optionally config.json).
    """
    base = Path(path)
    with open(base / "base_qa_pairs.json") as f:
        data = json.load(f)

    dialogues, labels = [], []
    metadata: dict[str, list[Any]] = {"topic": [], "subtopic": [], "question_type": [], "id": []}
